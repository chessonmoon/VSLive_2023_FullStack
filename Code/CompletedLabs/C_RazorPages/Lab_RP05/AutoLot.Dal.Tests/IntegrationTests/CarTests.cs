...
public class CarTests : BaseTest, IClassFixture<EnsureAutoLotDatabaseTestFixture>
{
    ...
    [Fact]
    public void ShouldNotGetTheLemonsUsingFromSqlOnUnmappedType()
    {
        var entity = Context.Model.FindEntityType($"{typeof(CustomerOrderViewModel).FullName}");
        var tableName = entity.GetViewName();
        var schemaName = entity.GetViewSchema() ?? "dbo";
        var sql = $@"SELECT * from {schemaName}.{tableName}";
        var query = Context.Database.SqlQueryRaw<CustomerOrderViewModel>(sql);
        var qs = query.ToQueryString();
        OutputHelper.WriteLine($"Query: {qs}");
